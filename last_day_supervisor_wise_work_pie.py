...
from tasks
                where task_date = CONVERT(varchar(10), getdate()-1,126) and asign_name like ?""", connection,
                                       params={emp_name})
...
plt.legend(patches, labels, loc='upper right', bbox_to_anchor=(1.35,.92),
           fontsize=9)
plt.text(0.1, 1.4, '10. Last Day Working Hour according to supervisor', ha='center',color='#3238a8', fontsize=14, fontweight='bold')
#plt.title('Working Hour Percentage Per Company',fontsize='14',color='white', fontweight='bold',backgroundcolor='#6a9aba')
plt.tight_layout()

# plt.show()
plt.savefig('Last_day_working_hour_according_to_supervisor.png')
plt.close()

print('10. Last Day Working Hour according to supervisor')