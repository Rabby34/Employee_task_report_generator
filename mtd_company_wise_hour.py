...
from tasks
where Year(task_date)  = year(CONVERT(varchar(10), getdate(),126))
and Month(task_date)  = Month(CONVERT(varchar(10), getdate(),126))
...
plt.text(0.1, 1.4, '5. MTD Working Hour Per Company', ha='center',color='#3238a8', fontsize=14, fontweight='bold')
#plt.title('Working Hour Percentage Per Company',fontsize='14',color='white', fontweight='bold',backgroundcolor='#6a9aba')
plt.tight_layout()

# plt.show()
plt.savefig('mtd_company_wise_hour.png')
plt.close()
print('5. MTD Working Hour Per Company generated')
